...
class GI_AI_API_Handler {
    ...
    public function __construct() {
        ...
        $this->temperature = max(0.0, min(2.0, floatval(get_option('gi_ai_temperature', 0.2))));
    ...
    private function get_system_prompt() {
        return "あなたは日本の助成金・補助金制度に精通した専門家です。実在する助成金制度の一般的なパターンを参考に、具体的で実用的な情報を生成してください。
...
- 「、、、」「。。。」「...」「…」などの記号連続
- 「とか」「など」「等」などの曖昧表現
- 「適切な」「十分な」「各種の」「様々な」などの抽象的表現
- 「○○等の業種」→「製造業、IT業、サービス業」に具体化
- 「各種書類」→「申請書、事業計画書、決算書」に具体化
- 「適当な金額」→「上限500万円（対象経費の50%）」に具体化
- 令和○年度、平成○年度の記載
- 実現不可能な条件や金額
...
具体的で実用的な、実際に申請したくなる助成金情報を作成してください。";
    ...
    /**
     * 概要プロンプト（改良版 - より具体的で実用的）
     */
    private function get_summary_prompt($base_info) {
        return "以下の情報から、助成金の魅力的な概要を200文字以内で作成してください：
...
上記の例のような、具体的で申請したくなる概要を作成してください：";
    ...
    private function get_target_prompt($base_info) {
        return "以下の情報から、助成金の対象者・対象事業を具体的に記載してください：

{$base_info}

【参考例】以下のような具体的な対象者情報を作成してください：

<h3>対象者</h3>
<ul>
<li><strong>事業者：</strong>中小企業者、個人事業主</li>
<li><strong>従業員数：</strong>5名以上100名以下</li>
<li><strong>年間売上：</strong>1億円以上10億円以下</li>
<li><strong>業種：</strong>製造業、IT関連業、サービス業、小売業</li>
<li><strong>地域：</strong>東京都内（または全国対応）</li>
<li><strong>その他条件：</strong>創業3年以上、法人税を納税していること</li>
</ul>

<h3>対象事業</h3>
<ul>
<li>新商品・新サービスの開発事業</li>
<li>生産性向上のためのIT導入事業</li>
<li>設備投資・機械購入事業</li>
<li>人材育成・研修事業</li>
<li>販路開拓・マーケティング事業</li>
</ul>

【作成ルール】
1. 必ず<h3>タグで「対象者」「対象事業」を分ける
2. <ul><li>形式で具体的な条件を列挙
3. 数値がある場合は具体的に記載（従業員数、売上高等）
4. 業種は日本標準産業分類を参考に具体的に記載
5. 地域は都道府県名または「全国」を明記
6. 申請者が「自分が該当するか」判断できる具体性を持たせる

上記例のような、実用的で分かりやすい対象者情報を作成してください：";
    }
...
300文字以内で、分かりやすい日本語で記述してください。";
    ...
    private function get_title_prompt($base_info) {
        return "以下の情報から、魅力的な助成金タイトルを40文字以内で作成してください：

{$base_info}

【良いタイトル例】
- 中小企業IT導入助成金 最大1000万円
- スタートアップ創業支援補助金 総額500万円
- 製造業設備投資助成金 助成率50%
- 店舗改修費用支援制度 上限300万円
- 人材育成支援補助金 最大100万円
...
【作成ルール】
1. 必ず「助成金」「補助金」「支援制度」のいずれかを含める
2. 対象業種または対象者を明記（中小企業、スタートアップ、製造業等）
3. 金額または助成率を具体的に記載
4. 40文字以内に収める
5. 検索されやすいキーワードを自然に含める
...
1,200文字以内で、ユーザーにとって有用でSEO効果の高い本文コンテンツを作成してください。";
    ...
    private function clean_generated_content($content, $field_name) {
        ...
        $content = preg_replace($ambiguous_patterns, $ambiguous_replacements, $content);
