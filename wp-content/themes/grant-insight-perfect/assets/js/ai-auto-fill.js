...
(function($) {
    ...
    class GI_AI_AutoFill {
        ...
        executeAIProcess(fields) {
            ...
            // 🔍 デバッグログ追加
            console.log('[AI Debug] 送信データ:', data);
            console.log('[AI Debug] AJAX URL:', gi_ai_ajax.ajax_url);
            console.log('[AI Debug] 選択フィールド:', fields);
            console.log('[AI Debug] 投稿ID:', postId);
        ...
        handleExecuteSuccess(response) {
            // 🔍 デバッグログ追加
            console.log('[AI Debug] レスポンス受信:', response);
            console.log('[AI Debug] 成功判定:', response.success);
            
            if (response.success) {
                const data = response.data;
                
                // 🔍 生成されたコンテンツの詳細ログ
                console.log('[AI Debug] 生成されたフィールド:', data.updated_fields);
                if (data.updated_fields) {
                    Object.entries(data.updated_fields).forEach(([field, content]) => {
                        console.log(`[AI Debug] ${field}:`, content.substring(0, 100) + '...');
                    });
                }
                ...
                // 🔍 エラー詳細ログ
                console.error('[AI Debug] 処理失敗:', response.data);
                console.error('[AI Debug] 完全レスポンス:', response);
        ...
        handleExecuteError(xhr, status, error) {
            // 🔍 詳細エラーログ
            console.error('[AI Debug] AJAX エラー詳細:');
            console.error('[AI Debug] Status:', status);
            console.error('[AI Debug] Error:', error);
            console.error('[AI Debug] Response Status:', xhr.status);
            console.error('[AI Debug] Response Text:', xhr.responseText);
            console.error('[AI Debug] XHR Object:', xhr);
            ...
            if (status === 'timeout') {
                ...
                errorMessage += 'サーバーエラー（詳細はコンソールログを確認）';
            } else {
